...
//  Capppy
//
//  Created by Jana Vac on 19.02.2023.
//
import UIKit
import Foundation
import FirebaseAuth
...
class UserApi {
    ...
    func signUp (withUsername username: String, email: String, password: String, image: UIImage?, onSuccess: @escaping() -> Void, onError: @escaping(_ errorMessage: String) -> Void) {
        Auth.auth().createUser(withEmail: email, password: password) { (authDataResult, error) in
            if error != nil {
                ProgressHUD.showError(ERROR_ALREG_EMAIL)
   ...
   //-----VÝMAZ
    ...
    func deleteAccount() {
            ...
            // Odstranění uživatelských dat z Realtime Database
            let userRef = Ref().databaseSpecificUser(uid: user.uid)
            userRef.removeValue { error, _ in
                if let error = error {
                    ProgressHUD.showError(ERROR_PERMISSIONS)
                    return
                }

                // Odstranění uživatelského účtu z Firebase autentifikace
                user.delete { error in
                    if let error = error {
                        ProgressHUD.showError(ERROR_LOGIN_AGAIN)
                        return
                    }
                    ...
                    // Vymazání účtu úspěšné, přepněte uživatele na úvodní obrazovku
                    (UIApplication.shared.delegate as! AppDelegate).configureInitialViewController()
                    ProgressHUD.showSuccess("Odstranění účtu proběhlo úspěšně")
